...
def _android_save_document(path, on_done, mime='application/json', then_share=False,
    ...
    def _on_result(request, result, intent):
        ...
        try:
            ...
            # نوشتنِ باینری‌امن تا فایل‌های ZIP هم سالم بمانند (نه فقط JSON متنی)
            ...
            if not wrote:
                ...
                bis = BufferedInputStream(fin, 65536)
                try:
                    b = bis.read()
                    while b != -1:
                        out.write(b)
...
def _desktop_save(path, on_done):
    """پنجرهٔ انتخابِ پوشهٔ مقصد برای ذخیرهٔ فایل روی دسکتاپ/ویندوز (کاربر خودش محل را انتخاب می‌کند)."""
    import os
    ...
    def _deliver(_t, msg):
        if on_done:
            on_done(False, msg or 'ذخیره لغو شد.')

    start = None
    for cand in ('/storage/emulated/0/Download', os.path.expanduser('~')):
        if os.path.isdir(cand):
            start = cand
            break
    _desktop_browse(_deliver, exts=('.zip', '.json'),
                    title='انتخابِ پوشهٔ مقصد برای ذخیره',
                    want_path=True, start_dir=start,
                    save_mode=True, on_save=_do, save_name=os.path.basename(path))


def _desktop_browse(deliver, exts=('.json',), title='انتخاب فایل', want_path=False, start_dir=None,
                    save_mode=False, on_save=None, save_name=''):
    ...
    def _list(path):
        ...
        if save_mode and on_save is not None:
            grid.add_widget(_btn('✔  ذخیره در همین پوشه: ' + (save_name or ''),
                                 lambda: (pop.dismiss(), on_save(state['cwd'])),
                                 bg=(0.13, 0.34, 0.20, 1)))
