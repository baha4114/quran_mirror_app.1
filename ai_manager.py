...
def build_note_messages(seed, target, operator):
    ...
    sys = (
        "تو پژوهشگری خبره در علومِ قرآنی و متخصصِ «پردازشِ آینه‌ایِ آیات» هستی. "
        "دو آیه به تو داده می‌شود که با یک عملگرِ آینه‌ای به هم پیوند خورده‌اند. "
        "یک تحلیلِ کوتاه، دقیق و روشن از رابطهٔ معناییِ این دو آیه بنویس: "
        "روی پیوندِ مضمونی، تقارن یا تضادِ معنایی تمرکز کن. "
        "خروجی فارسیِ روان و بی‌طرف، حداکثر ۴ تا ۶ جمله، بدون مقدمه‌چینی و بدون تکرارِ متنِ آیه باشد."
    )
    usr = "عملگرِ آینه‌ای: %s\n\n%s\n\n%s\n\nتحلیلِ کوتاه و دقیق را بنویس." % (
        (operator or "نامشخص"),
        _verse_block("آیهٔ بذر —", seed.get("s"), seed.get("a"), seed.get("arb"), seed.get("pers")),
        _verse_block("آیهٔ مقصد —", target.get("s"), target.get("a"), target.get("arb"), target.get("pers")),
    )
...
def _format_discoveries(favs, featured, cap=40):
    def fmt(items, label):
        ...
        for i, it in enumerate(items[:cap], 1):
            ...
            if len(note) > 220:
                note = note[:220] + "…"
            out.append("%d) [%s] سوره %s:%s ⟵ سوره %s:%s | برچسب: %s%s" % (
                i, (it.get("mode", "") or "-"),
