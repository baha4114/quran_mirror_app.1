...
def build_quran_chat_system():
    """سیستم‌پرامتِ گفت‌وگوی معمولیِ بخشِ قرآن (بدونِ بازیابیِ آیات) — برای سلام/گپ/راهنمایی."""
    nl = chr(10)
    return nl.join([
        "تو دستیارِ صمیمی و خوش‌برخوردِ بخشِ «گفت‌وگو دربارهٔ قرآن کریم» در اپلیکیشنِ «قطب‌نمای قرآنی» هستی.",
        "این پیامِ کاربر یک پرسشِ محتواییِ قرآنی نیست (مثلاً سلام، تشکر، گپِ کوتاه، یا پرسش دربارهٔ خودت و توانایی‌هایت).",
        "پس کوتاه، گرم و طبیعی پاسخ بده؛ جوابِ سلام را بده و با کاربر تعامل کن.",
        "اگر مناسب بود، یک‌خط یادآوری کن که می‌توانی در سراسرِ آیاتِ قرآن بگردی و تحلیل کنی، و بپرس دربارهٔ چه موضوع یا آیه‌ای می‌خواهد گفت‌وگو کند.",
        "هیچ آیه‌ای از خودت نقل نکن و ادعای عددی نکن. فارسیِ روان و بسیار کوتاه بنویس.",
    ])


def quran_classify(question, history=None, on_done=None, on_error=None):
    """تشخیصِ نیت: آیا پیام یک پرسشِ محتواییِ قرآنی است (نیاز به بازیابیِ آیات) یا گفت‌وگوی معمولی؟
    on_done(True) یعنی پرسشِ قرآنی؛ on_done(False) یعنی گفت‌وگوی معمولی."""
    sys = (
        "وظیفهٔ تو فقط دسته‌بندیِ پیامِ کاربر است. "
        "اگر پیام یک پرسش یا درخواستِ محتوایی دربارهٔ قرآن است (آیات، موضوعات، واژه‌ها، مفاهیم، تحلیل، جست‌وجوی آیه) پاسخ بده: QURAN. "
        "اگر پیام سلام، احوال‌پرسی، تشکر، خداحافظی، گپِ کوتاه، یا پرسشی دربارهٔ خودِ دستیار و توانایی‌هایش است پاسخ بده: CHAT. "
        "فقط و فقط یکی از دو کلمهٔ QURAN یا CHAT را بنویس، بدونِ هیچ توضیح."
    )
    msgs = [{"role": "system", "content": sys}]
    for h in (history or [])[-4:]:
        if isinstance(h, dict) and h.get("content"):
            msgs.append({"role": h.get("role", "user"), "content": str(h["content"])[:400]})
    msgs.append({"role": "user", "content": (question or "").strip()})

    def _parsed(text):
        t = _strip_think(text or "").strip().upper()
        if ("CHAT" in t) and ("QURAN" not in t):
            is_quran = False
        elif ("QURAN" in t) and ("CHAT" not in t):
            is_quran = True
        else:
            is_quran = True  # ابهام → پیش‌فرض بازیابیِ آیات
        if on_done:
            on_done(is_quran)

    chat(msgs, on_done=_parsed, on_error=on_error, stream=False,
