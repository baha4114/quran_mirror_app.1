...
def verse_card(mode, s, a, arb, pers, is_seed=False, is_fallback=False, reason='',
               on_save=None, score_text='', on_select=None, selected=False, hide_ref=False):
    ...
    head = RLabel(mode if hide_ref else f'{mode}   سوره {s} ، آیه {a}', bold=True, font_size='15sp',
...
class MatrixScreen(BaseScreen):
    ...
    def _make_card(self, idx, is_seed=False):
        ...
        # دکمه‌های «اطلاعات عددی» و «حذف موقت» به ستونِ پایینِ کارت منتقل شدند تا با
        # برچسبِ مکی/مدنی و انیمیشنِ اعداد تداخل نکنند و از کارت بیرون نزنند.
        # متنِ مختصات هم مثلِ قبل داخلِ «اطلاعات عددی» دیده می‌شود.
        ...
        # ستونِ دکمه‌ها: «اطلاعات عددی» و «حذف موقت» زیرِ هم و تمام‌عرضِ داخلِ کارت
        btn_col = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(40), spacing=dp(6))
        try:
            btn_col.add_widget(info_button([('', c['s'], c['a'])], full=True,
                                           height=dp(40), font_size='13sp'))
        except Exception:
            pass
        if not is_seed:
            xb = PillButton('حذف موقت', bg=C_RED, fg=(1, 1, 1, 1),
                            size_hint_y=None, height=dp(40), font_size='13sp')
            xb.bind(on_release=lambda *a, i=idx: self._hide_card(i))
            btn_col.add_widget(xb)
        btn_col.bind(minimum_height=lambda i, v: setattr(i, 'height', v))
        btn_col.height = btn_col.minimum_height
...
class PredictScreen(BaseScreen):
    ...
    def show(self, s, a, kind):
        ...
        if kind == 'semantic':
            preds = core.predict_mirror(app.data, s, a, seed['arb'], model=app.get_model())
            ...
            preds = core.predict_mirror_numeric(app.data, s, a, model=app.get_model())
            if not preds:
...
def lab_section_of(item):
    """بخش لابراتوار: بن‌بستی‌ها جدا، تردیدی‌ها جدا، گروهی‌ها جدا، بقیه زیر عملگر خودشان."""
    if item.get('source') == 'blind':
        return 'BLIND'
    if item.get('is_dead_end'):
        return 'DEAD'
...
def open_note_editor(item, source='lab', title='ویرایش تحلیل', intro=None, on_saved=None, saved_msg='ذخیره شد ✓', on_cancel=None):
    ...
    state = {'d': bool(item.get('is_doubtful', False)), 'x': bool(item.get('is_dead_end', False))}
    ...
    def _sync_status():
        # وقتی کشف «بن‌بست» است، تردید معنا ندارد و دکمهٔ وضعیت خاکستری و بی‌اثر می‌شود
        if state['x']:
            btog.set_text('وضعیت: مطمئن (بن‌بست)')
            btog._bg = list(C_MUTED)
        else:
    ...
    def _tog(*a):
        if state['x']:
            toast('این کشف «بن‌بست» است؛ اول بن‌بست را خاموش کنید.', 'وضعیت', kind='warn')
            return
        state['d'] = not state['d']
    ...
    # وضعیتِ سوم: «بن‌بست» — بذر با هیچ‌کدام از مقصدها ارتباطی نداشت و کاربر مطمئن است (نمونهٔ منفی)
    bdead = PillButton('⛔ بن‌بست: بله' if state['x'] else 'بن‌بست: خیر',
                       bg=C_RED if state['x'] else C_MUTED, size_hint_y=None, height=dp(44), font_size='14sp')
    def _tog_dead(*a):
        state['x'] = not state['x']
        if state['x']:
            state['d'] = False
        bdead.set_text('⛔ بن‌بست: بله' if state['x'] else 'بن‌بست: خیر')
        bdead._bg = list(C_RED if state['x'] else C_MUTED)
        bdead._state()
        _sync_status()
    bdead.bind(on_release=_tog_dead)
    content.add_widget(bdead)
    _sync_status()
...
def show_group_discovery(item, source='lab', screen=None):
    ...
    def _render():
        ...
        for idx, t in enumerate(targets):
            ...
            if t.get('verdict'):
                box.add_widget(_auto_label('نتیجهٔ آزمون کور: ' + str(t.get('verdict')),
                                           font_size='12sp', color=C_TEAL, halign='right'))
            if source == 'lab' and item.get('source') == 'blind':
                pmb = PillButton('انتقال به عملگر خودش', bg=C_GREEN, size_hint_y=None,
                                 height=dp(38), font_size='12sp')

                def _pm(*a, i=idx):
                    ok, msg = app.promote_blind_target(item, i)
                    toast(msg, 'انتقال' if ok else 'خطا')
                    if ok:
                        _refresh_parent()
                pmb.bind(on_release=_pm)
                box.add_widget(pmb)
            if source == 'lab':
...
def show_discovery(item, source='lab', screen=None):
    ...
    def _edit(*a):
        ...
        _st = {'d': bool(item.get('is_doubtful', False)), 'x': bool(item.get('is_dead_end', False))}
        ...
        def _sync_status2():
            if _st['x']:
                b_status.set_text('وضعیت: مطمئن (بن‌بست)')
                b_status._bg = list(C_MUTED)
            else:
        ...
        # تغییرِ وضعیتِ «بن‌بست» برای همین کشف (برای وقتی که بعداً تکلیفش روشن شد)
        b_dead = PillButton('⛔ بن‌بست: بله' if _st['x'] else 'بن‌بست: خیر',
                            bg=C_RED if _st['x'] else C_MUTED, size_hint_y=None, height=dp(44), font_size='14sp')

        def _tog_dead2(*a):
            _st['x'] = not _st['x']
            if _st['x']:
                _st['d'] = False
            b_dead.set_text('⛔ بن‌بست: بله' if _st['x'] else 'بن‌بست: خیر')
            b_dead._bg = list(C_RED if _st['x'] else C_MUTED)
            b_dead._state()
            _sync_status2()
        b_dead.bind(on_release=_tog_dead2)
        content.add_widget(b_dead)
        _sync_status2()
...
class StatsScreen(BaseScreen):
    """داشبورد پژوهش: آمار کشفیات، تفکیک نوع پردازش و مسیر، و قوی‌ترین نگاه‌های هر عملگر."""

    def __init__(self, **kw):
        super().__init__(title='داشبورد پژوهش', **kw)
        self.scroll = ScrollView(do_scroll_x=False, bar_width=dp(4))
        self.list = BoxLayout(orientation='vertical', size_hint_y=None, spacing=dp(10), padding=dp(4))
        self.list.bind(minimum_height=self.list.setter('height'))
        self.scroll.add_widget(self.list)
    ...
    def _pct(n, d):
        if not d:
            return '۰٪'
        return '%d٪' % int(round(100.0 * float(n) / float(d)))

    def _panel(self, title, lines, color=None):
        box = RoundBox(bg=(0.10, 0.14, 0.22, 1), border=(color or C_BLUE), orientation='vertical',
                       size_hint_y=None, padding=dp(10), spacing=dp(4))
        t = RLabel(title, bold=True, font_size='15sp', color=C_GOLD, halign='right', size_hint_y=None)
        t.bind(texture_size=lambda i, v: setattr(i, 'height', v[1] + dp(4)))
        box.add_widget(t)
        for ln in lines:
            lb = RLabel(ln, font_size='13sp', color=C_TEXT, halign='right', size_hint_y=None)
            lb.bind(texture_size=lambda i, v: setattr(i, 'height', v[1] + dp(4)))
            box.add_widget(lb)
        box.bind(minimum_height=lambda i, v: setattr(box, 'height', v + dp(20)))
        _fade_in(box)
        return box
    ...
    def refresh(self):
        ...
        self.list.clear_widgets()
        if not app.favs:
            self.list.add_widget(empty_state('هنوز کشفی ثبت نشده است',
                                             hint='با ثبت کشف در لابراتوار، آمار اینجا ساخته می‌شود'))
            return
        try:
            st = core.stats_summary(app.favs, app.data)
        except Exception:
            self.list.add_widget(empty_state('محاسبهٔ آمار ممکن نشد'))
            return
        model = app.get_model()
        n = st['pairs']
        self.list.add_widget(self._panel('وضعیت کلی', [
            'مجموع جفت‌های بذر و مقصد: %d' % n,
            'کشفیات قطعی: %d  (%s)' % (st['pos'], self._pct(st['pos'], n)),
            'کشفیات بن‌بستی: %d  (%s)' % (st['neg'], self._pct(st['neg'], n)),
            'کشفیات تردیدی: %d  (%s)' % (st['doubt'], self._pct(st['doubt'], n)),
        ], C_GOLD))

        proc_lines = []
        for key in (core.PROC_MIRROR, core.PROC_ROTATION, core.PROC_OTHER):
            d = st['by_proc'].get(key)
            if not d:
                continue
            proc_lines.append('%s: %d مورد  (%s از کل)'
                              % (core.PROC_TITLES.get(key, key), d['n'], self._pct(d['n'], n)))
            proc_lines.append('    قطعی %d · بن‌بست %d · تردیدی %d'
                              % (d['pos'], d['neg'], d['doubt']))
        rt = st['by_route']
        direct = rt.get('مستقیم', 0)
        circ = rt.get('گردش ساعتی', 0)
        tot_rt = direct + circ
        proc_lines.append('رسیدن مستقیم (بدون گردش ساعتی): %d  (%s)' % (direct, self._pct(direct, tot_rt)))
        proc_lines.append('رسیدن با گردش ساعتی: %d  (%s)' % (circ, self._pct(circ, tot_rt)))
        self.list.add_widget(self._panel('تفکیک نوع پردازش', proc_lines, C_PURPLE))

        b_tot = b_pos = b_neg = b_unk = 0
        b_sessions = 0
        d_tot = d_pos = 0
        for it in app.favs:
            if it.get('source') != 'blind':
                continue
            b_sessions += 1
            for t in it.get('decoys', []):
                d_tot += 1
                if t.get('verdict') == 'تأیید':
                    d_pos += 1
        ...
        if b_tot:
            blind_lines = [
                'تعداد آزمون‌ها: %d — مجموع مقصدهای داوری‌شده: %d' % (b_sessions, b_tot),
                'نرخ پایهٔ ارتباط (تأیید): %s' % self._pct(b_pos, b_tot),
                'بن‌بست: %d  (%s)' % (b_neg, self._pct(b_neg, b_tot)),
                'نامشخص: %d  (%s)' % (b_unk, self._pct(b_unk, b_tot)),
            ]
            if d_tot:
                _gap = (100.0 * b_pos / b_tot) - (100.0 * d_pos / d_tot)
                blind_lines.append('—')
                blind_lines.append('تله‌های کنترلی (آیات کاملاً تصادفی): %d مورد' % d_tot)
                blind_lines.append('نرخ تأییدِ تله‌ها: %s' % self._pct(d_pos, d_tot))
                blind_lines.append('فاصلهٔ قدرت (عملگرها منهای تصادف): %+d واحد' % int(round(_gap)))
                if _gap >= 25:
                    blind_lines.append('تفسیر: اثرِ عملگرها آشکارا از تصادف بیشتر است.')
                elif _gap >= 10:
                    blind_lines.append('تفسیر: نشانهٔ امیدوارکننده است؛ آزمون بیشتری لازم است.')
                else:
                    blind_lines.append('تفسیر: فعلاً فاصلهٔ معناداری دیده نمی‌شود.')
            else:
                blind_lines.append('هر نرخِ کشفِ بالاتر از این عدد، نشانهٔ اثرِ واقعیِ عملگرهاست.')
        else:
            blind_lines = ['هنوز آزمون کوری انجام نشده است.',
                           'از لابراتوار، «بذر تصادفی — آزمون کور» را بزن تا نرخ پایه ساخته شود.']
        self.list.add_widget(self._panel('نرخ پایه (آزمون کور)', blind_lines, C_TEAL))

        op_names = dict(OPERATORS)
        for key, _t in OPERATORS:
            d = st['by_op'].get(key)
            if not d:
                continue
            lines = ['تعداد: %d — قطعی %d (%s) · بن‌بست %d · تردیدی %d'
                     % (d['n'], d['pos'], self._pct(d['pos'], d['n']), d['neg'], d['doubt']),
                     'با گردش ساعتی: %d  (%s)' % (d['fb'], self._pct(d['fb'], d['n']))]
            if model is not None:
                try:
                    lines.append('اعتماد مدل به این عملگر: %d٪' % int(round(model.confidence(key) * 100)))
                    for r in model.top_lenses(key, 4):
                        sign = 'به‌سودِ ارتباط' if r['power'] > 0 else 'به‌زیانِ ارتباط'
                        lines.append('• %s = %s  (%s · قطعی %d / بن‌بست %d)'
                                     % (r['title'], r['value'], sign, int(r['pos']), int(r['neg'])))
                    kws = model.top_keywords(key, 6)
                    if kws:
                        lines.append('واژه‌های پرتکرارِ فضای این عملگر: ' + '، '.join(w for w, _c in kws))
                    rels = model.top_relations(key, 3)
                    if rels:
                        lines.append('رفتارِ غالب: ' + '، '.join('%s (%d)' % (r, int(c)) for r, c in rels))
                except Exception:
                    pass
            self.list.add_widget(self._panel(op_names.get(key, key), lines, C_BLUE))


class BlindScreen(BaseScreen):
    """بذر تصادفی و آزمون کور: مقصدها با ترتیبِ تصادفی و بدونِ نامِ عملگر داوری می‌شوند."""

    def __init__(self, **kw):
        super().__init__(title='بذر تصادفی — آزمون کور', **kw)
    ...
    def new_session(self, *a):
        ...
        seed = None
        for _ in range(300):
            s = _random.randint(1, 114)
            mx = app.data.max_a.get(s, 0)
            if not mx:
                continue
            a2 = _random.randint(1, mx)
            d = app.data.get(s, a2)
            if d:
                seed = {'s': s, 'a': a2, 'arb': d.get('arb', ''), 'pers': d.get('pers', '')}
                break
        if seed is None:
            toast('بذر تصادفی پیدا نشد.', 'خطا')
            return
        targets = []
        try:
            cards = core.process_matrix(app.data, seed['s'], seed['a'])
        except Exception:
            cards = []
        for c in cards:
            if c.get('kind') != 'target':
                continue
            targets.append({'s': c.get('s'), 'a': c.get('a'), 'arb': c.get('arb', ''),
                            'pers': c.get('pers', ''), 'operator': c.get('mode', ''),
                            'is_fallback': bool(c.get('is_fallback', False)),
                            'is_decoy': False, 'verdict': 'نامشخص'})
        # تله‌های کنترلی: آیاتِ کاملاً تصادفی، بدون هیچ عملگری
        n_decoy = 3 if len(targets) >= 5 else 2
        used = set([(seed['s'], seed['a'])] + [(t['s'], t['a']) for t in targets])
        guard = 0
        while sum(1 for t in targets if t.get('is_decoy')) < n_decoy and guard < 800:
            guard += 1
            s3 = _random.randint(1, 114)
            mx3 = app.data.max_a.get(s3, 0)
            if not mx3:
                continue
            a3 = _random.randint(1, mx3)
            if (s3, a3) in used:
                continue
            d3 = app.data.get(s3, a3)
            if not d3:
                continue
            used.add((s3, a3))
            targets.append({'s': s3, 'a': a3, 'arb': d3.get('arb', ''), 'pers': d3.get('pers', ''),
                            'operator': '', 'is_fallback': False, 'is_decoy': True,
                            'verdict': 'نامشخص'})
        _random.shuffle(targets)
        self._session = {'seed': seed, 'targets': targets, 'i': 0, 'saved': False}
        self._render()

    def _answer(self, verdict):
        se = self._session
        if not se or se['i'] >= len(se['targets']):
            return
        se['targets'][se['i']]['verdict'] = verdict
        se['i'] += 1
        self._render()
    ...
    def _render(self):
        ...
        self.list.clear_widgets()
        if not se:
            self.list.add_widget(empty_state('برای شروع، آزمون تازه بگیر'))
            return
        seed = se['seed']
        total = len(se['targets'])
        idx = se['i']
        self.list.add_widget(verse_card('بذر تصادفی — نشانی پنهان', seed['s'], seed['a'],
                                        seed['arb'], seed['pers'], is_seed=True, hide_ref=True))
        if idx < total:
            self.info.set_text('مقصد %d از %d — نامِ عملگر و نشانی آیه پنهان است' % (idx + 1, total))
            t = se['targets'][idx]
            self.list.add_widget(verse_card('مقصد ناشناس %d' % (idx + 1), t['s'], t['a'],
                                            t.get('arb', ''), t.get('pers', ''), hide_ref=True))
            row = BoxLayout(size_hint_y=None, height=dp(48), spacing=dp(8))
            b_ok = PillButton('ارتباط دارد', bg=C_GREEN, font_size='13sp')
            b_ok.bind(on_release=lambda *a: self._answer('تأیید'))
            b_no = PillButton('بن‌بست', bg=C_RED, font_size='13sp')
            b_no.bind(on_release=lambda *a: self._answer('بن‌بست'))
            row.add_widget(b_ok)
            row.add_widget(b_no)
            self.list.add_widget(row)
            b_skip = PillButton('نمی‌دانم — رد شو', bg=(1, 1, 1, 0.14),
                                size_hint_y=None, height=dp(44), font_size='13sp')
            b_skip.bind(on_release=lambda *a: self._answer('نامشخص'))
            self.list.add_widget(b_skip)
            return
        real = [t for t in se['targets'] if not t.get('is_decoy')]
        dec = [t for t in se['targets'] if t.get('is_decoy')]
        vs = [t.get('verdict', 'نامشخص') for t in real]
        ds = [t.get('verdict', 'نامشخص') for t in dec]
        self.info.set_text('آزمون تمام شد — نتیجه را ثبت کن')
        card = RoundBox(bg=(0.12, 0.10, 0.20, 1), border=C_TEAL, orientation='vertical',
                        size_hint_y=None, padding=dp(10), spacing=dp(4))
        _p_real = int(round(100.0 * vs.count('تأیید') / len(vs))) if vs else 0
        _p_dec = int(round(100.0 * ds.count('تأیید') / len(ds))) if ds else 0
        _lines = ['مقصدهای عملگری: %d تأیید · %d بن‌بست · %d نامشخص  (نرخ تأیید: %d٪)'
                  % (vs.count('تأیید'), vs.count('بن‌بست'), vs.count('نامشخص'), _p_real)]
        if ds:
            _lines.append('تله‌های کنترلی (آیات کاملاً تصادفی): %d تأیید از %d  (نرخ تأیید: %d٪)'
                          % (ds.count('تأیید'), len(ds), _p_dec))
            _lines.append('فاصلهٔ قدرت: %+d واحد — هرچه بزرگ‌تر، اثرِ واقعیِ عملگرها قوی‌تر.'
                          % (_p_real - _p_dec))
            _lines.append('تله‌ها در لابراتوار به عنوان کشف ثبت نمی‌شوند؛ فقط معیار سنجش‌اند.')
        _lines.append('با ثبت، این آزمون به شکلِ یک مجموعه در «کشفیات آزمون کور» ذخیره می‌شود')
        _lines.append('و نامِ عملگر و نشانیِ هر مقصد در همان‌جا آشکار می‌گردد.')
        for ln in _lines:
            lb = RLabel(ln, font_size='13sp', color=C_TEXT, halign='right', size_hint_y=None)
            lb.bind(texture_size=lambda i, v: setattr(i, 'height', v[1] + dp(4)))
            card.add_widget(lb)
        ...
        b_save = PillButton('ثبت این آزمون در لابراتوار', bg=C_GOLD,
                            size_hint_y=None, height=dp(48), font_size='14sp')
        b_save.bind(on_release=lambda *a: self._save())
        self.list.add_widget(b_save)
        b_new = PillButton('آزمون تازه با بذر تصادفی دیگر', bg=C_INDIGO,
                           size_hint_y=None, height=dp(46), font_size='13sp')
        b_new.bind(on_release=self.new_session)
        self.list.add_widget(b_new)

    def _save(self):
        se = self._session
        if not se or se.get('saved'):
            return
        app = App.get_running_app()
        app.add_blind_session(se['seed'],
                              [t for t in se['targets'] if not t.get('is_decoy')],
                              decoys=[t for t in se['targets'] if t.get('is_decoy')])
        se['saved'] = True
        self._session = None
        toast('آزمون کور ثبت شد ✓', 'لابراتوار')
        self.manager.transition = SlideTransition(direction='right')
        self.manager.current = 'lab'
        try:
            self.manager.get_screen('lab').refresh()
        except Exception:
            pass
...
class LabScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        self.body(row2)
        row_r = BoxLayout(size_hint_y=None, height=dp(46), spacing=dp(8))
        b_stats = PillButton('داشبورد پژوهش', bg=C_BLUE, font_size='13sp')
        b_stats.bind(on_release=lambda *a: self.open_stats())
        b_csv = PillButton('خروجی CSV دیتاست', bg=C_GREEN, font_size='13sp')
        b_csv.bind(on_release=lambda *a: self.export_dataset_csv())
        row_r.add_widget(b_stats)
        row_r.add_widget(b_csv)
        self.body(row_r)
        row_b = BoxLayout(size_hint_y=None, height=dp(48), spacing=dp(8))
        b_blind = PillButton('بذر تصادفی — آزمون کور', bg=C_INDIGO, font_size='14sp')
        b_blind.bind(on_release=lambda *a: self.open_blind())
        row_b.add_widget(b_blind)
        self.body(row_b)
        try:
            apply_glow(b_blind, C_INDIGO, speed=0.7, width=4.0, hi=0.9, lo=0.2)
        except Exception:
            pass
    ...
    def open_stats(self):
        scr = self.manager.get_screen('stats')
        scr.refresh()
        self.manager.transition = SlideTransition(direction='left')
        self.manager.current = 'stats'

    def open_blind(self):
        scr = self.manager.get_screen('blind')
        scr.new_session()
        self.manager.transition = SlideTransition(direction='left')
        self.manager.current = 'blind'

    def export_dataset_csv(self):
        app = App.get_running_app()
        try:
            path = app.export_dataset_csv()
        except Exception:
            path = None
        if not path:
            toast('هنوز داده‌ای برای خروجی وجود ندارد.', 'خطا')
            return
        try:
            import share_util
            share_util.save_file_to_device(
                path, on_done=lambda ok, msg: toast(msg, 'دیتاست' if ok else 'خطا'),
                mime='text/csv', then_share=True)
        except Exception:
            toast('فایل ساخته شد: %s' % os.path.basename(path), 'دیتاست')
...
class OperatorScreen(BaseScreen):
    ...
    def _card(self, item):
        ...
        if _tp:
            ...
            card.add_widget(tr)
        extra = '  (بن‌بست)' if item.get('is_dead_end') else ('  (تردیدی)' if item.get('is_doubtful') else '')
    ...
    def _group_card(self, item):
        ...
        card.add_widget(head)
        if item.get('source') == 'blind':
            _vs = [t.get('verdict', 'نامشخص') for t in item.get('all_targets', [])]
            sub = RLabel('%d تأیید · %d بن‌بست · %d نامشخص'
                         % (_vs.count('تأیید'), _vs.count('بن‌بست'), _vs.count('نامشخص')),
                         font_size='12sp', color=C_TEAL, halign='center', size_hint_y=None)
            sub.bind(texture_size=lambda i, v: setattr(i, 'height', v[1] + dp(4)))
            card.add_widget(sub)
...
# چند ثانیه پیش از پایانِ کلیپ، ترنزیشنِ ورود آغاز شود (جلوگیری از فریزِ آخرین فریم)
INTRO_HANDOFF = 0.45


class LazyScreenManager(ScreenManager):
    """مدیریتِ صفحه‌ها با ساختِ تنبل: هر صفحه فقط در نخستین ورودِ کاربر ساخته می‌شود.

    رفتار و ظاهرِ اپ دقیقاً مثلِ قبل است؛ فقط زمانِ راه‌اندازی و مصرفِ حافظه کمتر می‌شود،
    چون در شروع فقط صفحهٔ خانه ساخته می‌شود و بقیه هنگامِ نیاز.
    """
    ...
    def add_factories(self, mapping):
        self._factories.update(mapping or {})

    def built_screen(self, name):
        """اگر صفحه قبلاً ساخته شده باشد برمی‌گرداند؛ وگرنه None (بدونِ ساختن)."""
    ...
    def ensure_screen(self, name):
        s = self.built_screen(name)
        if s is not None:
            return s
        fac = self._factories.pop(name, None)
        if fac is None:
            return None
        s = fac()
        s.name = name
    ...
    def has_screen(self, name):
        return self.built_screen(name) is not None or name in self._factories
...
class QuranMirrorApp(App):
    def build(self):
        ...
        # صفحات
        sm = LazyScreenManager(transition=FadeTransition(duration=0.25))
        # فقط صفحهٔ خانه همین حالا ساخته می‌شود؛ بقیهٔ صفحه‌ها در نخستین ورودِ کاربر
        # ساخته می‌شوند (ظاهر و رفتار هیچ تغییری نمی‌کند، فقط اجرای اولیه سبک‌تر می‌شود).
        sm.add_widget(HomeScreen(name='home'))
        sm.add_factories({
            'matrix': lambda: MatrixScreen(name='matrix'),
            'predict': lambda: PredictScreen(name='predict'),
            'lab': lambda: LabScreen(name='lab'),
            'operator': lambda: OperatorScreen(name='operator'),
            'featured': lambda: FeaturedScreen(name='featured'),
            'search': lambda: SearchScreen(name='search'),
            'tags': lambda: TagsScreen(name='tags'),
            'media': lambda: MediaScreen(name='media'),
            'backup': lambda: BackupScreen(name='backup'),
            'about': lambda: AboutScreen(name='about'),
            'guide': lambda: GuideScreen(name='guide'),
            'chat_disc': lambda: ChatScreen(name='chat_disc', mode='discoveries'),
            'chat_quran': lambda: ChatScreen(name='chat_quran', mode='quran'),
            'experiment': lambda: ExperimentScreen(name='experiment'),
            'stats': lambda: StatsScreen(name='stats'),
            'blind': lambda: BlindScreen(name='blind'),
        })
        self.sm = sm
    ...
    def _play_intro_video(self, on_done):
        ...
        def _finish(*a):
            ...
            if ov is not None:
                try:
                    # ورودِ باشکوه به اپ (به‌جای فیدِ سادهٔ قبلی)
                    self._grand_enter(ov, _remove)
                except Exception:
                    _remove()
            else:
                _remove()
        ...
        try:
            ...
            vid = Video(source=path, state='play', options={'eos': 'stop'})  # پایانِ کلیپ = پایان، نه تکرار
            try:
            ...
            self._intro_finish = _finish

            # پایانِ واقعیِ کلیپ هم تشخیص داده می‌شود (نه فقط سقفِ زمانی)
            def _on_eos(*a):
                _finish()
            ...
            def _on_state(inst, val):
                if val == 'stop':
                    _finish()
            try:
                vid.bind(eos=_on_eos)
            except Exception:
                pass
            try:
                vid.bind(state=_on_state)
            except Exception:
                pass
            ...
            def _poll(dt):
                ...
                if st['started'] and (now - st['start_wall']) >= max(0.4, target - INTRO_HANDOFF):
                    _finish()
                    return False
                return True
            st['poll_ev'] = Clock.schedule_interval(_poll, 0.05)
    ...
    def _play_intro_video_native(self, on_done):
        ...
        def _remove_native():
            try:
                ...
                pass
            # برداشتنِ پرچمِ «صفحه روشن بماند» پس از پایانِ کلیپ
            try:
                _PA = autoclass('org.kivy.android.PythonActivity')
                _WMLP = autoclass('android.view.WindowManager$LayoutParams')
                _PA.mActivity.getWindow().clearFlags(_WMLP.FLAG_KEEP_SCREEN_ON)
            except Exception:
                pass

        def _end(*a):
            # ترتیب مهم است: نخست سطحِ ویدئوی نیتیو برداشته می‌شود، سپس ترنزیشن اجرا می‌شود.
            # وگرنه فلاشِ طلایی زیرِ سطحِ ویدئو می‌ماند و اصلاً دیده نمی‌شود.
            if done['v'] or state.get('ending'):
                return
            state['ending'] = True
            _remove_native()
            # فرصتِ کوتاه به رابطِ اندروید تا سطحِ ویدئو واقعاً برداشته شود
            try:
                Clock.schedule_once(lambda *b: _proceed_kivy(), 0.12)
                # تورِ ایمنی: اگر آن زمان‌بندی به هر دلیل اجرا نشد
                Clock.schedule_once(lambda *b: _proceed_kivy(), 0.9)
            except Exception:
                _proceed_kivy()

        self._intro_finish = _end

        # سقفِ زمانی بر پایهٔ مدتِ واقعیِ کلیپ (اگر خوانده شود)؛ وگرنه سقفِ ایمنِ ۲۰ ثانیه
        cap = {'ev': None}

        def _set_cap(seconds):
            def _apply(*a):
                ev = cap.get('ev')
                if ev is not None:
                    try:
                        ...
                        pass
                cap['ev'] = Clock.schedule_once(lambda *b: _end(), float(seconds))
            ...
            def onPrepared(self, mp):
                try:
                    ...
                    if vv is not None:
                        ...
                        vv.start()
                    try:
                        # لحظهٔ تحویل را کمی پیش از آخرین فریم می‌گذاریم تا تصویرِ پایانی فریز نشود
                        # و ترنزیشنِ طلایی دقیقاً روی حرکتِ پایانیِ کلیپ سوار شود (ورودِ یکدست)
                        _dur = float(mp.getDuration() or 0) / 1000.0
                        if _dur > 1.0:
                            _set_cap(max(0.4, _dur - INTRO_HANDOFF))
                        elif _dur > 0.0:
                            _set_cap(_dur)
                    except Exception:
                        pass
        ...
        def _start():
            try:
                ...
                # در حینِ پخشِ کلیپ، صفحه خاموش/کم‌نور نشود
                try:
                    _WMLP = autoclass('android.view.WindowManager$LayoutParams')
                    activity.getWindow().addFlags(_WMLP.FLAG_KEEP_SCREEN_ON)
                except Exception:
                    pass
        ...
        # safety cap: if the real duration is unknown and completion never fires
        _set_cap(20)

    def _stop_intro_if_running(self):
        """اگر وسطِ پخشِ کلیپ، اپ به پس‌زمینه رفت، پخش تمیز بسته می‌شود
        تا صدا در پس‌زمینه نماند و ورود به اپ درست انجام شود."""
        fn = getattr(self, '_intro_finish', None)
        if fn is None:
            return
        self._intro_finish = None
        try:
    ...
    def _grand_enter(self, overlay, on_done=None):
        """ورودِ باشکوه به اپ پس از کلیپِ معرفی (فقط نخستین اجرا).

        یک فلاشِ طلاییِ نرم به همراهِ دو حلقهٔ نورانی که از مرکز باز می‌شوند،
        پردهٔ مشکی را کنار می‌زند و صفحهٔ خانه با فیدِ نرم (out_expo) بالا می‌آید.
        رویِ خودِ کلیپ و اندازهٔ آن هیچ اثری ندارد؛ فقط لحظهٔ تحویلِ کلیپ به اپ را می‌سازد.
        """
        from kivy.graphics import Line as _Line
        st = {'t': 0.0, 'ev': None, 'done': False}
        try:
            W, H = Window.size
            W = float(W or 0) or 1080.0
            H = float(H or 0) or 1920.0
        except Exception:
            W, H = 1080.0, 1920.0
        cx, cy = W / 2.0, H / 2.0
        rmax = ((W * W + H * H) ** 0.5) / 2.0 + dp(40)
        DUR = 1.45

        flash = Widget(size_hint=(None, None), size=(W, H), pos=(0, 0))
        with flash.canvas:
            f_col = Color(C_GOLD[0], C_GOLD[1], C_GOLD[2], 0.0)
            Rectangle(pos=(0, 0), size=(W, H))
            r1_col = Color(1, 1, 1, 0.0)
            ring1 = _Line(circle=(cx, cy, dp(10)), width=dp(2))
            r2_col = Color(C_GOLD[0], C_GOLD[1], C_GOLD[2], 0.0)
            ring2 = _Line(circle=(cx, cy, dp(10)), width=dp(1.2))
            r3_col = Color(1, 0.93, 0.66, 0.0)
            ring3 = _Line(circle=(cx, cy, dp(10)), width=dp(0.9))
        flash.bind(on_touch_down=lambda *a: True)
        Window.add_widget(flash)

        # صفحهٔ اپ از پشتِ پرده با فیدِ نرم نمایان می‌شود
        sm = getattr(self, 'sm', None)
        if sm is not None:
            try:
                sm.opacity = 0
                Animation(opacity=1, d=1.30, t='out_expo').start(sm)
            except Exception:
                pass
        ...
        def _step(dt):
            ...
            if p > 1.0:
                p = 1.0
            try:
                # فلاشِ طلایی: سریع روشن، آرام محو
                if p < 0.18:
                    f_col.a = 0.5 * (p / 0.18)
                else:
                    f_col.a = 0.5 * (max(0.0, 1.0 - (p - 0.18) / 0.82) ** 1.6)
                # حلقهٔ اول: از مرکز به بیرون
                e1 = 1.0 - (1.0 - p) ** 3
                ring1.circle = (cx, cy, dp(10) + rmax * e1)
                r1_col.a = 0.6 * (max(0.0, 1.0 - p) ** 1.2)
                # حلقهٔ دوم: کمی عقب‌تر و طلایی
                q = 0.0 if p <= 0.14 else (p - 0.14) / 0.86
                e2 = 1.0 - (1.0 - q) ** 3
                ring2.circle = (cx, cy, dp(10) + rmax * e2)
                r2_col.a = 0.45 * (max(0.0, 1.0 - p) ** 1.2)
                # حلقهٔ سوم: آرام‌تر و روشن‌تر، برای عمق دادن به ورود
                q3 = 0.0 if p <= 0.30 else (p - 0.30) / 0.70
                e3 = 1.0 - (1.0 - q3) ** 3
                ring3.circle = (cx, cy, dp(10) + rmax * e3)
                r3_col.a = 0.35 * (max(0.0, 1.0 - p) ** 1.2)
            except Exception:
                pass
            if p >= 1.0:
                _end()
        ...
        # پردهٔ مشکی همزمان و نرم کنار می‌رود
        if overlay is not None:
            try:
                Animation(opacity=0, d=0.85, t='out_expo').start(overlay)
            except Exception:
                pass
        st['ev'] = Clock.schedule_interval(_step, 1 / 60.0)
        # تورِ ایمنی: در هر حالت، فلاش برداشته می‌شود و کار ادامه پیدا می‌کند
        Clock.schedule_once(_end, DUR + 0.5)
    ...
    def _autosave_chat_on_exit(self):
        for _nm in ('chat_disc', 'chat_quran'):
            try:
                # فقط صفحه‌هایی که واقعاً ساخته شده‌اند (با ساختِ تنبل، نباید هنگامِ خروج ساخته شوند)
                _get = getattr(self.sm, 'built_screen', None)
                scr = _get(_nm) if callable(_get) else self.sm.get_screen(_nm)
                if scr is None:
                    continue
    ...
    def _build_featured_groups(self):
        ...
        for it in self.featured:
            ...
            rec = {
                ...
                'is_dead_end': bool(it.get('is_dead_end', False)),
    ...
    def _clean_by_operator(self, items):
        ...
        for it in items:
            ...
            if it.get('decoys'):
                entry['control_decoys'] = [{'sura': t.get('s'), 'ayah': t.get('a'),
                                            'arabic': t.get('arb', ''),
                                            'translation': t.get('pers', ''),
                                            'verdict': t.get('verdict', '')}
                                           for t in it.get('decoys', [])]
            if 'all_targets' in it:
                entry['targets'] = [{'sura': t.get('s'), 'ayah': t.get('a'),
                                     'arabic': t.get('arb', ''), 'translation': t.get('pers', ''),
                                     'operator': t.get('operator', ''),
                                     'verdict': t.get('verdict', '')}
                                    for t in it.get('all_targets', [])]
    ...
    # ---------- مدلِ یادگیرنده و دیتاست ----------
    def get_model(self, force=False):
        """مدلِ یادگیری از روی کشفیات لابراتوار (تا وقتی کشفی تغییر نکرده، همان مدل دوباره به‌کار می‌رود)."""
        try:
            sig = (len(self.favs),
                   sum(1 for it in self.favs if it.get('is_dead_end')),
                   sum(1 for it in self.favs if it.get('is_doubtful')),
                   sum(len(it.get('all_targets', [])) for it in self.favs))
        except Exception:
            sig = None
        if force or getattr(self, '_model', None) is None or getattr(self, '_model_sig', None) != sig:
            try:
                self._model = core.learn_from_discoveries(self.favs, self.data)
            except Exception:
                self._model = None
            self._model_sig = sig
    ...
    def export_dataset_csv(self):
        """خروجی CSV دیتاست: هر جفتِ بذر و مقصد یک ردیف با همهٔ نگاه‌های عددی."""
        try:
            rows = core.dataset_rows(self.favs, self.data)
        except Exception:
            rows = []
        if not rows:
            return None
        import csv as _csv
        lens_keys = [k for k, _t in core.LENS_DEFS]
        head = ['operator', 'processing', 'mode', 'seed_s', 'seed_a', 'target_s', 'target_a',
                'label', 'is_fallback', 'relation_type', 'date', 'source',
                'seed_arb', 'target_arb', 'note'] + lens_keys
        out = self._p('dataset_%s.csv' % datetime.now().strftime('%Y%m%d_%H%M'))
        with open(out, 'w', newline='', encoding='utf-8-sig') as f:
            w = _csv.writer(f)
            w.writerow(head)
            for r in rows:
                lens = r.get('lenses', {}) or {}
                w.writerow([r.get('op', ''),
                            core.PROC_TITLES.get(r.get('proc'), r.get('proc', '')),
                            r.get('mode', ''), r.get('S'), r.get('A'), r.get('s'), r.get('a'),
                            r.get('label'), 1 if r.get('is_fallback') else 0,
                            r.get('relation_type', ''), r.get('date', ''), r.get('source', ''),
                            r.get('seed_arb', ''), r.get('target_arb', ''),
                            (r.get('note', '') or '').replace(chr(10), ' ')]
                           + [lens.get(k, '') for k in lens_keys])
        return out

    # ---------- آزمون کور (بذر تصادفی) ----------
    def add_blind_session(self, seed, targets, decoys=None):
        """یک آزمون کور را به شکلِ یک مجموعه در لابراتوار ثبت می‌کند."""
        entry = {
            'mode': 'آزمون کور', 'source': 'blind',
            'seed_s': seed['s'], 'seed_a': seed['a'],
            'seed_arb': seed.get('arb', ''), 'seed_pers': seed.get('pers', ''),
            'all_targets': [{'s': t.get('s'), 'a': t.get('a'), 'arb': t.get('arb', ''),
                             'pers': t.get('pers', ''), 'operator': t.get('operator', ''),
                             'verdict': t.get('verdict', 'نامشخص'),
                             'is_fallback': bool(t.get('is_fallback', False))}
                            for t in (targets or [])],
            'decoys': [{'s': t.get('s'), 'a': t.get('a'), 'arb': t.get('arb', ''),
                        'pers': t.get('pers', ''), 'verdict': t.get('verdict', 'نامشخص')}
                       for t in (decoys or [])],
            'note': '', 'relation_type': 'نامشخص',
            'is_doubtful': False, 'is_dead_end': False,
            'date': datetime.now().strftime('%Y-%m-%d %H:%M'),
        }
        self.favs.append(entry)
        self.save_favs()
    ...
    def promote_blind_target(self, item, idx):
        """یک مقصدِ آزمون کور را به بخشِ عملگرِ خودش منتقل می‌کند."""
        try:
            t = (item.get('all_targets') or [])[idx]
        except Exception:
            return False, 'مقصد پیدا نشد.'
        mode_name = normalize_mode(t.get('operator', '') or '')
        entry = {
            'mode': mode_name,
            'seed_s': item.get('seed_s'), 'seed_a': item.get('seed_a'),
            'seed_arb': item.get('seed_arb', ''), 'seed_pers': item.get('seed_pers', ''),
            'target_s': t.get('s'), 'target_a': t.get('a'),
            'target_arb': t.get('arb', ''), 'target_pers': t.get('pers', ''),
            'note': item.get('note', ''), 'relation_type': item.get('relation_type', 'نامشخص'),
            'is_doubtful': (t.get('verdict') == 'نامشخص'),
            'is_dead_end': (t.get('verdict') == 'بن‌بست'),
            'is_fallback': bool(t.get('is_fallback', False)),
            'source': 'blind_promoted',
            'date': datetime.now().strftime('%Y-%m-%d %H:%M'),
        }
        for it in self.favs:
            if (it.get('seed_s'), it.get('seed_a'), it.get('target_s'), it.get('target_a'),
                    it.get('mode')) == (entry['seed_s'], entry['seed_a'], entry['target_s'],
                                        entry['target_a'], entry['mode']):
                return False, 'این کشف قبلاً ثبت شده است.'
        self.favs.append(entry)
        self.save_favs()
        self.last_discovery_key = discovery_key(entry)
        self.last_discovery_section = lab_section_of(entry)
        self.get_model(force=True)
        return True, 'به بخشِ عملگرِ خودش منتقل شد ✓'
    ...
    # ---------- کشف گروهی و جفتی ----------
    def add_group_discovery(self, seed, targets, note='', relation_type='نامشخص', is_doubtful=False, is_dead_end=False):
        ...
        open_note_editor(entry, 'lab', title='ثبت تحلیل کشف گروهی',
                         intro='کشف گروهی با %d مقصد ثبت شد. تحلیل، وضعیت تردید یا «بن‌بست» را انتخاب کنید:' % len(targets),
                         on_saved=lambda: setattr(self, 'last_discovery_section', lab_section_of(entry)),
                         on_cancel=lambda: self._discard_discovery(entry),
                         saved_msg='کشف گروهی با موفقیت ثبت شد ✓')

    def add_pair_discovery(self, seed, ta, tb, note='', relation_type='نامشخص', is_doubtful=False, is_dead_end=False):
    ...
    def _import_item_list(self, items, target='lab', mode='merge'):
        ...
        for it in items:
            ...
            it.setdefault('is_dead_end', False)
