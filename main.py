...
class HomeScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        # شهابِ نورانی مثلِ کادرِ بالا، اما کمی کوچک‌تر و با رنگِ متفاوت
        orbit_dot(vsbox, C_BLUE, scale=0.7)
        ...
        orbit_dot(self.verse_box, C_ORANGE, scale=0.7)
        ...
        orbit_dot(seedbox, C_GREEN, scale=0.7)
        ...
        for label, color, target in nav:
            ...
            # شهابِ نورانیِ کوچک‌تر (تیپر) دورِ هر دکمهٔ ناوبری، هم‌رنگِ خودِ دکمه
            orbit_dot(b, color, scale=0.45, fps=30)
...
def orbit_dot(widget, color=None, period=5.5, scale=1.0, fps=60):
    ...
    def _render():
        ...
        for idx, t in enumerate(targets):
            ...
            if source == 'lab':
                ...
                box.add_widget(rmb)
        box.add_widget(_auto_label('رفتار ��بکه: ' + str(item.get('relation_type', 'نامشخص')),
                                   font_size='14sp', color=C_ORANGE, halign='right'))
        _note = item.get('note', '')
...
def show_discovery(item, source='lab', screen=None):
    ...
    grid = GridLayout(cols=2, size_hint_y=None, height=dp(104), spacing=dp(8))
    if source == 'lab':
        bb = PillButton('ا��زودن به گلچین', bg=C_GOLD, font_size='14sp')
        bb.bind(on_release=_to_featured)
        grid.add_widget(bb)
    be = PillButton('ویرایش تحلیل', bg=C_BLUE, font_size='14sp')
    be.bind(on_release=_edit)
...
class QuranMirrorApp(App):
    ...
    def import_from_path(self, path, target='lab', mode='merge'):
        ...
        try:
            ...
            return (0, 0, 'خطا ��ر بارگذاری: %s' % str(e)[:80])
