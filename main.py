...
class HomeScreen(BaseScreen):
    ...
    def _show_verse_result(self, s, a):
        ...
        self.verse_meta.set_text('سوره %s (نزول %s) ، آیه %s' % (s, qref.nuzul(s), a))
    ...
    def _build_results_popup(self, q, results):
        ...
        def _make_row(r):
            ...
            card.add_widget(RLabel('سوره %s (نزول %s) ، آیه %s' % (s, qref.nuzul(s), a), font_size='12sp',
...
def verse_card(mode, s, a, arb, pers, is_seed=False, is_fallback=False, reason='',
    ...
    head = RLabel(f'{mode}   سوره {s} (نزول {qref.nuzul(s)}) ، آیه {a}', bold=True, font_size='15sp',
...
class MatrixScreen(BaseScreen):
    ...
    def show(self, s, a):
        ...
        self._reset_state()
        self._seed = (s, a)
        self.title_label.set_text('پردازش : سوره %s (نزول %s) و آیه %s' % (s, qref.nuzul(s), a))
    ...
    def _ai_analyze(self):
        ...
        show_ai_result_popup('تحلیل یکپارچهٔ ماتریس', msgs,
                             subtitle='بذر: سوره %s (نزول %s) ، آیه %s  —  %d مقصد' % (seed['s'], qref.nuzul(seed['s']), seed['a'], len(targets)))
    ...
    def _make_card(self, idx, is_seed=False):
        ...
        head = RLabel('%s   سوره %s (نزول %s) ، آیه %s' % (c.get('mode', ''), c['s'], qref.nuzul(c['s']), c['a']), bold=True,
...
def _verse_block(border, s, a, arb, pers):
    ...
    c.add_widget(_auto_label('سوره %s (نزول %s) ، آیه %s' % (s, qref.nuzul(s), a), font_size='12sp', color=C_MUTED, halign='right'))
...
class OperatorScreen(BaseScreen):
    ...
    def _card(self, item):
        ...
        card = RoundBox(bg=(0.10, 0.14, 0.22, 1), border=border, orientation='vertical',
                        size_hint_y=None, padding=dp(10), spacing=dp(4))
        pair = RLabel('سوره %s:%s (نزول %s)     سوره %s:%s (نزول %s)' % (item.get('seed_s'), item.get('seed_a'),
                      qref.nuzul(item.get('seed_s')), item.get('target_s'), item.get('target_a'), qref.nuzul(item.get('target_s'))),
    ...
    def _group_card(self, item):
        ...
        card = RoundBox(bg=(0.12, 0.10, 0.20, 1), border=border, orientation='vertical',
                        size_hint_y=None, padding=dp(10), spacing=dp(4))
        head = RLabel('%s — سوره %s:%s (نزول %s) ← %d مقصد' % (item.get('mode', ''), item.get('seed_s'),
                      item.get('seed_a'), qref.nuzul(item.get('seed_s')), len(item.get('all_targets', []))),
...
class SearchResultCard(RecycleDataViewBehavior, ClickCard):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        self._top.set_text('[%s]  سوره %s:%s (نزول %s) ↔ سوره %s:%s (نزول %s)  %s' % (
            tag, it.get('seed_s'), it.get('seed_a'), qref.nuzul(it.get('seed_s')),
            it.get('target_s'), it.get('target_a'), qref.nuzul(it.get('target_s')), it.get('mode', '') or ''))
...
class SearchScreen(BaseScreen):
    ...
    def _result_row(self, src, it):
        ...
        top = RLabel('[%s]  سوره %s:%s (نزول %s) ↔ سوره %s:%s (نزول %s)  %s' % (
            tag, it.get('seed_s'), it.get('seed_a'), qref.nuzul(it.get('seed_s')),
            it.get('target_s'), it.get('target_a'), qref.nuzul(it.get('target_s')), it.get('mode', '') or ''),
