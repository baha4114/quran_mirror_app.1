...
def open_note_editor(item, source='lab', title='ویرایش تحلیل', intro=None, on_saved=None, saved_msg='ذخیره شد ✓'):
    ...
    def _ai_fill(*a):
        ...
        def _err(msg):
            ...
            toast(msg, 'هوش مصنوع��', kind='error')
...
def show_discovery(item, source='lab', screen=None):
    ...
    be.bind(on_release=_edit)
    grid.add_widget(be)
    bd = PillButton('حذف کشف' if source == 'lab' else 'حذف از گل��ین', bg=C_RED, font_size='14sp')
    bd.bind(on_release=_delete)
    grid.add_widget(bd)
...
class ChatScreen(BaseScreen):
    def __init__(self, **kw):
        super().__init__(title='گفت��و با هوش مصنوعی', **kw)
...
class QuranMirrorApp(App):
    ...
    def on_start(self):
        try:
            ...
            pass
        # --- تشخیصِ موتورِ متن: آیا Pango فعال شد؟ و اگر نه، چرا؟ ---
        try:
            from kivy.core.text import Label as _CTL
            _eng = (getattr(_CTL, '__module__', '') or '')
            _is_pango = 'pango' in _eng.lower()
            if _is_pango:
                _msg = 'موتور متن: Pango ✓ فعال است'
                _kind = 'success'
            else:
                try:
                    import importlib
                    importlib.import_module('kivy.core.text.text_pango')
                    _why = 'کامپایل شده ولی بارگذاری نشد'
                except Exception as _pe:
                    _pes = str(_pe)
                    if ('No module named' in _pes) or ('cannot import name' in _pes):
                        _why = 'اصلاً کامپایل نشده'
                    else:
                        _why = _pes[:70]
                _msg = 'موتور متن: sdl2 (Pango نیامد)\nعلت: ' + _why
                _kind = 'warn'
            Clock.schedule_once(lambda *a: toast(_msg, 'تشخیصِ موتورِ متن', kind=_kind), 1.4)
        except Exception:
            pass
