...
class QuranMirrorApp(App):
    ...
    def on_start(self):
        # جریانِ شروع:
        #   • بارِ اول (پس از نصب): ویدئویِ معرفی intro.m4a تمام‌صفحه پخش می‌شود،
        ...
        try:
            ...
            if first_run:
                ...
                try:
                    ...
                    pass
                self._play_intro_video(_after_intro)
            else:
    ...
    def _play_intro_video(self, on_done):
        """ویدئویِ معرفی را تمام‌صفحه رویِ همهٔ اپ پخش می‌کند و در پایان با فِید کنار می‌رود.
        اگر ffpyplayer/ویدئو در دسترس نبود یا فایل پخش نشد، بی‌سروصدا واردِ اپ می‌شود."""
        done = {'v': False}
        ...
        try:
            ...
            vid = Video(source=path, state='play', options={'eos': 'stop'},
                        size_hint=(1, 1), pos_hint={'x': 0, 'y': 0})
            try:
                ...
                pass
            overlay.add_widget(vid)

            # لمسِ صفحه = ردکردنِ ویدئو (skip)
            ...
            # پایانِ ویدئو: هم رویداد eos، هم تغییرِ وضعیت به stop
            vid.bind(eos=lambda inst, val: _finish() if val else None)
            vid.bind(state=lambda inst, val: _finish() if val == 'stop' else None)

            # وقتی طولِ ویدئو معلوم شد، کمی پس از پایانش هم تضمینی ببند
            def _on_dur(inst, val):
                try:
                    if val and float(val) > 0:
                        Clock.schedule_once(_finish, float(val) + 1.0)
                except Exception:
                    pass
            vid.bind(duration=_on_dur)

            # نگهبان: اگر بعد از ۸ ثانیه ویدئو اصلاً شروع نشده بود (نبودِ ffpyplayer)، واردِ اپ شو
            def _watchdog(*a):
                if done['v']:
                    return
                try:
                    pos = float(vid.position or 0)
                    dur = float(vid.duration or 0)
                except Exception:
                    pos, dur = 0.0, 0.0
                if dur <= 0 and pos <= 0:
                    _finish()
    ...
    def _play_open_curtain(self, on_done=None):
        """از دومین اجرا به بعد: دو لتِ هم‌رنگِ پس‌زمینه که از وسط به چپ و راست
        باز می‌شوند و صفحهٔ اپ را نمایان می‌کنند (افکتِ باز شدنِ پنجره)."""
        try:
            W, H = Window.size
            if not W or not H:
                if on_done:
                    on_done()
                return
            col = (0.051, 0.078, 0.141, 1)  # #0d1424
            overlay = FloatLayout(size_hint=(None, None), size=(W, H), pos=(0, 0))

            left = Widget(size_hint=(None, None), size=(W / 2.0 + 1, H), pos=(0, 0))
            with left.canvas:
                Color(*col)
            ...
            right = Widget(size_hint=(None, None), size=(W / 2.0 + 1, H), pos=(W / 2.0, 0))
            with right.canvas:
                Color(*col)
            ...
            Animation(x=-(W / 2.0) - 2, d=0.55, t='out_cubic').start(left)
            aR = Animation(x=W, d=0.55, t='out_cubic')
