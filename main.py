...
class QuranMirrorApp(App):
    ...
    def on_start(self):
        try:
            ...
            pass
        # --- تشخیصِ موتورِ متن: آیا Pango فعال شد؟ (برای بیلدِ آزمایشی) ---
        try:
            from kivy.core.text import Label as _CTL
            _is_pango = 'pango' in (getattr(_CTL, '__module__', '') or '').lower()
            Clock.schedule_once(
                lambda *a: toast(
                    ('موتور متن: Pango ✓ فعال است' if _is_pango
                     else 'موتور متن: sdl2 — Pango فعال نشد'),
                    'تشخیصِ موتورِ متن',
                    kind=('success' if _is_pango else 'warn')),
                1.4)
        except Exception:
            pass
