...
class RoundBox(BoxLayout):
    """جعبهٔ گوشه‌گرد با پس‌زمینه، سایهٔ نرمِ شناو�� و لبهٔ شیشه‌ای."""
...
def show_html_in_app(raw_html):
    ...
    # دلیل: روی ��ندروید یک برچسبِ بسیار بلند، یک بافتِ (texture) گرافیکیِ غول‌آسا می‌سازد
...
class PersianTextInput(_KbFocusMixin, TextInput):
        ...
        try:
            if self.multiline:
                ...
                if disp and disp != self._logical:
                    ...
                    # بدونِ وارون������ی (شکل‌دهیِ بومی یا متنِ لاتین/عددی): تازه‌ترین حرف در انتهاست
    ...
    def insert_text(self, substring, from_undo=False):
        ...
        # رفع کرش: اگر کیبوردِ ��ن������روید متنِ خالی فرستاد، رندر و جستجو را بی‌دلیل اجرا نکن
        if not substring:
            return None
...
class PersianEditor(_KbFocusMixin, TextInput):
    ...
    def delete_selection(self, from_undo=False):
        ...
        # را ا�� مدلِ منطقیِ من عبور می‌دهیم تا دو مدل از هم نیفتند.
        if self._native:
...
class TransformChip(BoxLayout):
    ...
    # اول رویِ اعدادِ بذر (به رنگِ طلاییِ عنوان) مکث می‌کند، بعد شروع به ت��ییر.
...
class HomeScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        # پنل دستیار هوش مصنوعی: ورود به گفتگو + تنظی��ات کلید API
...
def pulse_aura(widget, color=None, speed=2.2, thickness=1.0, hi=0.12, lo=0.03,
    ...
    # حاشیهٔ نورانیِ «بهم‌پیوسته» که دورِ کلِ کادر ��اموش/روشن می‌تپد.
...
class MatrixScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        self._cards = []
        self._view_a = {}             # idx -> آیهٔ نمایش‌داده‌شده (��اوبری مستقل کارت)
...
class PredictScreen(BaseScreen):
    ...
    def _open_html(self):
        ...
        content.add_widget(RLabel('��د HTML گزارش هوش مصنوعی را اینجا بچسبانید:',
...
def _neon_badge(text, color=None, size=None):
    """نشان ک��چک نئونی چشمک‌زن (مثلاً T1..T7) برای گوشهٔ کارت."""
...
def open_big_analysis_editor(initial_text, on_save):
    ...
    root.add_widget(RLabel('در کادرِ پایین بنویس؛ نمایشِ درست و متصل را بالا زند�� می‌بینی.',
...
def _add_analysis_field(content, initial_text=''):
    """در پنجرهٔ تحلیل: نمایشِ درستِ فقط‌خواندن��ِ تحلیل + دکمهٔ «ویرایش تحلیل».
...
def open_note_editor(item, source='lab', title='ویرایش تحلیل', intro=None, on_saved=None, saved_msg='ذخیره شد ✓', on_cancel=None):
    ...
    def _tog(*a):
        ...
        # رنگ پس‌��مینه هم همگام شود تا تغییر وضعیت کاملاً دیده شود
...
def show_group_discovery(item, source='lab', screen=None):
    ...
    def _edit(*a):
        open_note_editor(item, source, title='ویرایش ��حلیل', on_saved=_render)
...
def show_discovery(item, source='lab', screen=None):
    ...
    grid = GridLayout(cols=2, size_hint_y=None, height=dp(104), spacing=dp(8))
    if source == 'lab':
        bb = PillButton('افزودن به گ��چین', bg=C_GOLD, font_size='14sp')
        bb.bind(on_release=_to_featured)
        grid.add_widget(bb)
    be = PillButton('ویرایش تحلیل', bg=C_BLUE, font_size='14sp')
    be.bind(on_release=_edit)
...
def _exp_attach_dialog(new_item, desc=''):
    ...
    sv.add_widget(grid)
    root.add_widget(sv)
    p = Popup(title=P('افزود�� به آز��ایش'), content=root, size_hint=(0.94, 0.86),
              title_font='ui', title_align='center', separator_color=C_GOLD)
...
def _exp_pick_discovery_dialog(exp, on_added=None):
    ...
    def _add(it, src):
        ...
        if on_added:
            on_added()
        toast('کشف به ��زمایش وصل شد ✓' if ok else 'این کشف از قبل در آزمایش هست.',
              'دفتر پژوهش', kind=None if ok else 'warn')
...
def _exp_semantic_system(exp):
    ...
    role = nl.join([
        ...
        'این گفت‌وگو کاملاً مخصوصِ همین آزمایش است و به هیچ بخش ی�� گفت‌وگوی دیگری ربطی ندارد.',
...
def open_experiment_tasks(on_pick=None, kind='numeric'):
    ...
    root.add_widget(b_new)
    root.add_widget(RLabel('روی «در�� در چت» بزن تا تسک به گفت‌وگو برود' if on_pick else 'تسک‌های ریاضی را اینجا مدیریت کن',
...
def open_experiment_ai(exp, screen=None, mode='numeric'):
    ...
    # مهاجرتِ ��اریخچهٔ نسخهٔ قبل (که فقط یک چتِ عددی داشت)
...
class LabScreen(BaseScreen):
    ...
    def clear_all(self):
        ...
        def _do():
            ...
            app.save_favs()
            self.refresh()
        confirm('کل کشفیات ��ابراتوار پاک شود؟', _do, 'پاک کردن کل')
...
class TagsScreen(BaseScreen):
    DEFAULT = ["پژواک واژگانی", "تقارن ساختاری/نحوی", "هم‌آوایی صوتی", "تقابل کامل", "مکمل و بسط‌دهنده", "علت و معلول", "پرسش و پاسخ", "دیالو�� متقاطع", "گفت و گو", "وعده و تحقق وعده", "تمثیل موازی", "دادگاه و اعتراف", "تسبیح کائنات", "تکمیل پازل داستانی", "زاویه دید متفاوت"]
...
class GuideScreen(BaseScreen):
    ...
    SECTIONS = [
        ...
        ('۷) گلچین برگزیده', C_GOLD,
         'ویترینِ بهترین کشف‌های شما.\nکشف‌های مهم را از لابراتوار به گلچین می‌آورید. اینجا هم مثلِ لابراتوار بر اساسِ همان عملگرها �� دسته‌ها (گروهی و تردیدی) مرتب شده و به‌صورتِ روان بارگذاری می‌شود.\nمی‌توانید از کلِ گلچین یک خروجیِ تمیزِ JSON بگیرید.'),
        ...
        ('۱۰) رسانه و معرفی', C_ORANGE,
         'در این بخش «چند کلام از طراح» را می‌شنوید.\nیک معرفیِ صوتیِ کوتاه هنگامِ باز شدنِ برنامه یک‌بار پخش می‌شود و از اینجا هم ��ی‌توانید دوباره آن را بشنوید.'),
...
def _open_url(url):
    ...
    try:
        ...
        toast('نشد س��یت باز شود؛ این آدرس را دستی باز کن:\n' + url, 'راهنما')
...
class ChatScreen(BaseScreen):
    ...
    def _run_quran_turn(self, question, on_delta, on_done, on_error):
        """دومرحله‌ای: (۱) گرفتنِ کلیدواژه‌های مرتب�� از هوش مصنوعی، (۲) جست‌وجوی گستردهٔ آیات
        در ک��ِ دیتاکاوش، (۳) تحلیلِ زندهٔ آیاتِ یافت‌شده با تأکید بر جانیفتادنِ اشاره‌های مستقیم و غیرمستقیم."""
    ...
    def _load_chat(self, chat, popup=None):
        if getattr(self, '_busy', False):
            toast('صبر ک�� تا پاسخِ جاری کامل شود.', 'گفت‌وگو')
            return
...
class QuranMirrorApp(App):
    ...
    # ---------- ذخیره‌سازی ----------
    # ---------- چرخهٔ حیات: ذخیرهٔ خودکار�� گفت‌وگو هنگامِ بستن/توقفِ اپ ----------
    ...
    def delete_tag(self, tag):
        """حذفِ کاملِ یک برچسب (پیش‌فرض یا ک��ربر) و برداشتنِ آن از همهٔ کشف‌ها."""
    ...
    def add_pair_discovery(self, seed, ta, tb, note='', relation_type='ن��مشخص', is_doubtful=False):
    ...
    def import_from_path(self, path, target='lab', mode='merge'):
        ...
        try:
            ...
            if head[:2] == b'PK':
                ...
                # اگر فق�� نسخهٔ خوانا داخلِ ZIP بود
