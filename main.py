...
def _add_analysis_field(content, initial_text=''):
    ...
    زیر موتورِ sdl2 (که RTL بومی ندارد)، متنِ داخلِ کادرِ ویرایش هنگامِ تا��پ
...
def show_discovery(item, source='lab', screen=None):
    ...
    def _edit(*a):
        ...
        b_status = PillButton('وضعی��: ت��دیدی' if _st['d'] else 'وضعیت: مطمئن',
...
class ChatScreen(BaseScreen):
    def __init__(self, **kw):
        super().__init__(title='گفت��و ��ا هوش ��صنوعی', **kw)
...
class QuranMirrorApp(App):
    def build(self):
        ...
        try:
            ...
            pass
        # تور ایمنی: خطاهای پیش‌بینی‌نشده به‌جای بستنِ کامل برنامه ناد��ده گرفته ��وند
        try:
    ...
    def _parse_json_tolerant(self, text):
        ...
        BOM و JSON خط‌به‌خط مقاوم است. (payload, error) برمی‌گرد��ند."""
