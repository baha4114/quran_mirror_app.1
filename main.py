...
class FeaturedScreen(BaseScreen):
    ...
    def _go_chat(self):
        ...
        if hasattr(scr, 'set_mode'):
            scr.set_mode('discoveries')
        elif hasattr(scr, 'refresh'):
            scr.refresh()
        self.manager.transition = SlideTransition(direction='left')
...
class AboutScreen(BaseScreen):
    ...
    def __init__(self, **kw):
        ...
        box.add_widget(self._lbl('کاوش الگوهای عددی و معنایی میان آیات قرآن کریم. تمام ۶۲۳۶ آیه به صورت آفلاین در اپ گنجانده شده است در نظر داشته باشید سنگ بنای این اپلیکیشن سلیقه شخصی یا گمان و حدس نیست! این اپلیکیشن و بستر آن کاملا منطبق بر یافته های عددی بی سابقه قرآن کریم و موجود در پایگاه داده اصلی است که در زیر آن را مشاهده میکنید .', font_size='13sp', color=C_MUTED, halign='center'))
...
class ChatScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        self.mode = 'discoveries'  # حالتِ گفت‌وگو: 'discoveries' (کشفیات) یا 'quran' (قرآن کریم)
        try:
            self.title_label._fit_single = True
    ...
    def _send(self):
        ...
        if getattr(self, 'mode', 'discoveries') == 'quran':
            self._run_quran_turn((q or hist or '').strip(), _delta, _done, _err)
            return
