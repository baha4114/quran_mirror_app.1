    ...
    def refresh(self):
        ...
        if b_tot:
            blind_lines = [
                ...
                'هر نرخِ کشفِ بالاتر از این عدد، نشانهٔ اثرِ واقعیِ عملگرهاست.',
            ]
        else:
...
class BlindScreen(BaseScreen):
    ...
    def _render(self):
        ...
        idx = se['i']
        self.list.add_widget(verse_card('بذر تصادفی', seed['s'], seed['a'],
                                        seed['arb'], seed['pers'], is_seed=True))
        if idx < total:
            self.info.set_text('مقصد %d از %d — نامِ عملگر پنهان است' % (idx + 1, total))
            t = se['targets'][idx]
            self.list.add_widget(verse_card('مقصد ناشناس %d' % (idx + 1), t['s'], t['a'],
                                            t.get('arb', ''), t.get('pers', '')))
            ...
            return
        vs = [t.get('verdict', 'نامشخص') for t in se['targets']]
        self.info.set_text('آزمون تمام شد — نتیجه را ثبت کن')
        card = RoundBox(bg=(0.12, 0.10, 0.20, 1), border=C_TEAL, orientation='vertical',
                        size_hint_y=None, padding=dp(10), spacing=dp(4))
        for ln in ['نتیجهٔ این آزمون: %d تأیید · %d بن‌بست · %d نامشخص'
                   % (vs.count('تأیید'), vs.count('بن‌بست'), vs.count('نامشخص')),
                   'با ثبت، این آزمون به شکلِ یک مجموعه در «کشفیات آزمون کور» ذخیره می‌شود',
                   'و نامِ عملگرِ هر مقصد در همان‌جا آشکار می‌گردد.']:
            lb = RLabel(ln, font_size='13sp', color=C_TEXT, halign='right', size_hint_y=None)
            lb.bind(texture_size=lambda i, v: setattr(i, 'height', v[1] + dp(4)))
            card.add_widget(lb)
    ...
    def _save(self):
        ...
        if not se or se.get('saved'):
            return
        app = App.get_running_app()
        app.add_blind_session(se['seed'], se['targets'])
        se['saved'] = True
        self._session = None
...
class QuranMirrorApp(App):
    ...
    def add_blind_session(self, seed, targets):
