...
_NATIVE_SHAPING = None


def _use_native_shaping():
    """اگر ارائه‌دهندهٔ متنِ Kivy از نوع Pango باشد، شکل‌دهیِ دستی لازم نیست (خودِ Pango متن را شکل می‌دهد)."""
    global _NATIVE_SHAPING
    if _NATIVE_SHAPING is None:
        try:
            from kivy.core.text import Label as _CoreTextLabel
            _NATIVE_SHAPING = 'pango' in (getattr(_CoreTextLabel, '__module__', '') or '').lower()
        except Exception:
            _NATIVE_SHAPING = False
    return _NATIVE_SHAPING
